...
if [[ ! -f "${DOL}" ]]; then
  echo "Missing ${DOL}. Build with ./scripts/build-wii-gx-demo-docker.sh" >&2
  exit 1
fi
DOL="$(cd "$(dirname "${DOL}")" && pwd)/$(basename "${DOL}")"

if [[ -f "${REPO_ROOT}/scripts/validate-wii-dol.py" ]]; then
  if ! python3 "${REPO_ROOT}/scripts/validate-wii-dol.py" "${DOL}"; then
    echo "Padding DOL for Dolphin IPL alignment..." >&2
    python3 "${REPO_ROOT}/scripts/pad-wii-dol.py" "${DOL}"
